...
class DataBase:
    ...
    def insert(self, lst_in: str):
        for _ in lst_in:
            self.lst_data.append(dict(zip(list(self.FIELDS), _.split(" "))))

    def select(self, a: int, b: int):
        return [_[1] for _ in filter(lambda x: (x[0] >= a) and (x[0] <= b), enumerate(self.lst_data))]
