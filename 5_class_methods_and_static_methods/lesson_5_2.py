...
class Field:
    ...
    @classmethod
    def get_html_class(cls) -> str:
        return cls.HTML_CLASSES.get(cls.__name__, '')

    def get_html(self) -> str:
        return f"<p class='{self.get_html_class()}'>{self.name}: <input type='text' size={self.size} />"

    @classmethod
    def check_name(cls, name: str):
        if not all(char in cls.CHARS_CORRECT for char in name) or not (3 <= len(name) <= 50):
            raise ValueError("некорректное поле name")
